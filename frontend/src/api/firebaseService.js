 ...
 * Safely streams plant telemetry to Google Firebase Firestore & Cloud Database.
 * All credentials securely read from environment variables with graceful in-memory fallback.
 */

// Read config from Vite environment variables, falling back to safe demo identifiers
...
const DEFAULT_FIREBASE_CONFIG = {
  projectId: getEnv('VITE_FIREBASE_PROJECT_ID', 'helios-scada-cloud'),
  apiKey: getEnv('VITE_FIREBASE_API_KEY', ''),
  authDomain: getEnv('VITE_FIREBASE_AUTH_DOMAIN', 'helios-scada-cloud.firebaseapp.com'),
  storageBucket: getEnv('VITE_FIREBASE_STORAGE_BUCKET', 'helios-scada-cloud.appspot.com'),
  region: getEnv('VITE_FIREBASE_REGION', 'us-central1 (Firebase Cloud Firestore)'),
};
...
export function getFirebaseConfig() {
  try {
    ...
    return saved ? { ...DEFAULT_FIREBASE_CONFIG, ...JSON.parse(saved) } : DEFAULT_FIREBASE_CONFIG;
  } catch (e) {
...
export async function pushTelemetryToFirebase(record) {
  try {
    ...
    // Fail silently in offline / demo conditions
    return null;
  }
}
